...
from app.services.translation_service import TranslationProviderError, TranslationService
...
def test_rejects_too_many_translation_strings():
    with pytest.raises(ValueError, match="maximum translatable strings"):
        TranslationService().unzip({"items": ["one", "two"]}, max_strings=1)
