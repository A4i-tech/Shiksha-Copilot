...
class TranslationService:
    ...
    async def translate(self, data: dict[str, Any], src: str, dst: str) -> dict[str, Any]:
        ...
        for i in range(0, len(texts), self.batch_size):
            translated += await translator.translate_batch_async(texts[i:i+self.batch_size], src, dst)
        if len(translated) != len(flat):
            raise TranslationProviderError(f"Translator returned {len(translated)} results for {len(flat)} inputs")
        return self.zip(data, zip((p for p, _ in flat), translated))
    ...
    def unzip(self, data: dict[str, Any], max_strings: int = 1_000) -> list[tuple[tuple, str]]:
        out, stack = [], [((), data)]
        while stack:
            ...
            match value:
                case str():
                    if len(out) >= max_strings:
                        raise ValueError(f"JSON exceeds maximum translatable strings ({max_strings})")
