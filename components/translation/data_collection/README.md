...
This script and the cleanup script in next step require you to put the details of the GPT model azure deploymnet in a `.env` file. Make sure the following variables are set in that `.env` file: `AZURE_ENDPOINT` (exdpoint of gpt model on azure), `GPT_MODEL_NAME` (name of the hosted model, eg. GPT-5.1), `GPT_MODEL_ENGINE` (name of the hosted model, eg. GPT-5.1).
