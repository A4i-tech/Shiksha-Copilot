...
const { isAuthenticated, requireAnyPermission, requirePermission, requireUnscopedPermission } = require("../middlewares/auth");
...
router.get("/roles", isAuthenticated, requireAnyPermission("role.view", "role.assign", "user.create"), asyncMiddleware(controller.getAll.bind(controller)));
router.get("/roles/:id/users", isAuthenticated, requirePermission("role.view"), asyncMiddleware(controller.users.bind(controller)));
