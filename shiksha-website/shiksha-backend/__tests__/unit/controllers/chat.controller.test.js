...
describe('ChatController', () => {
    ...
    describe('sendMessage', () => {
        it('should send message successfully', async () => {
            ...
            mockChatManager.sendMessage.mockResolvedValue(mockResult);

            await chatController.sendMessage(mockReq, mockRes);

            expect(mockChatManager.sendMessage).toHaveBeenCalledWith('user123', 'Hello');
        ...
        it('should return 404 when sending fails', async () => {
            mockReq.body = { message: 'Hello' };
            const mockResult = { success: false, message: 'Failed', data: null };
            mockChatManager.sendMessage.mockResolvedValue(mockResult);
