#!/usr/bin/env python3
"""
ETL: MongoDB prod_dump2 -> analytics PostgreSQL (Superset star schema)
Full reload each run: truncate all tables, then rebuild from MongoDB.

Env vars:
  MONGO_URI  - mongodb://host:port/dbname  (default: mongodb://localhost:27017/prod_dump2)
  PG_DSN     - postgresql://user:pass@host:port/db  (default: analytics/analytics@localhost/analytics)
"""
import os
import sys
...
MONGO_URI = os.environ.get("MONGO_URI", "mongodb://localhost:27017/prod_dump2")
...
def main() -> None:
    ...
    log(f"  {len(state_id)} states, {len(dist_id)} districts, {len(block_id)} blocks")

    # ------------------------------------------------------------------ dim_schools
    log("Syncing dim_schools ...")
    ...
    log(f"  {len(mongo_school_to_pg)} schools")

    # ------------------------------------------------------------------ dim_users
    log("Syncing dim_users ...")
    ...
    # Teachers from users collection: role["standard"/"power"] → HM
    ...
    # Admins from adminusers collection: role["admin"] → StateAdmin
    for u in db.adminusers.find({"isDeleted": {"$ne": True}}):
        _insert_user(str(u["_id"]), u.get("name", "Unknown"), "StateAdmin",
                     None, fallback_region)

    log(f"  {len(valid_user_ids)} users")
    if not valid_user_ids:
        log("WARNING: no valid users — fact tables will be empty")
        pg.commit()
        return
    ...
    log(f"  {len(lp_rows)} lesson plans")

    # ------------------------------------------------------------------ fact_user_activities
    log("Syncing fact_user_activities ...")
    act_rows = []
    ...
    log(f"  {len(act_rows)} user activities")

    # ------------------------------------------------------------------ fact_ai_actions
    # Source: useractivities where moduleName is an AI feature (not useractions which only has login)
    log("Syncing fact_ai_actions ...")
    ai_rows = []
    ...
    log(f"  {len(ai_rows)} AI actions")

    # ------------------------------------------------------------------ fact_chatbot_sessions
    # resolved = True when session has >= 3 messages (substantive conversation)
    log("Syncing fact_chatbot_sessions ...")
    ...
    log(f"  {len(chat_rows)} chatbot sessions")

    # ------------------------------------------------------------------ fact_lba_attempts
    # lessonfeedbacks has no numeric scores (assessment array always empty).
    # Instead, use useractivities for AI-generation modules:
    #   score = min(interactionTime / 3.0, 100)  — time spent as engagement proxy (0-100)
    #   subject = AI feature category label
    log("Syncing fact_lba_attempts ...")
    lba_rows = []
    ...
    log(f"  {len(lba_rows)} LBA attempts")
