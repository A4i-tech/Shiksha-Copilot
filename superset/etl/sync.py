...
def main() -> None:
    ...
    for u in db.users.find({"isDeleted": {"$ne": True}}):
        ...
        if not region:
            continue
        school_pg = mongo_school_to_pg.get(str(u.get("school")))
        uid       = str(u["_id"])
        cur.execute(
            "INSERT INTO dim_users (user_id, name, role, school_id, region_id) "
            "VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
            (uid, u.get("name", "Unknown"), mapped, school_pg, region)
        )
    ...
    ai_rows = []
    for a in db.useractions.find({}):
        uid = str(a.get("userId", ""))
        if uid not in valid_user_ids:
            continue
        ai_rows.append((uid, a.get("actionType", "unknown"), coerce_dt(a.get("createdAt"))))
    ...
    for (tid, _), dates in sessions.items():
        ...
        chat_rows.append((tid, len(dates), True, created))
    ...
    lba_rows = []
    for fb in db.lessonfeedbacks.find({"isCompleted": True}):
        uid = str(fb.get("teacherId", ""))
        if uid not in valid_user_ids:
            continue
        meta    = lessons.get(str(fb.get("lessonId", "")), {})
        subject = meta.get("subject", "General")
        grade   = meta.get("grade", 1)

        score = 50.0
        for entry in (fb.get("assessment") or []):
            for key in ("score", "marks", "percentage", "totalScore"):
                v = entry.get(key)
                if v is not None:
                    try:
                        score = float(v)
        ...
        lba_rows.append((uid, subject, grade, score, coerce_dt(fb.get("createdAt"))))
