"""POSTs period-plan payloads to the lesson-plan Durable Functions API and polls each to completion.
Kept outside OmniIngest: POST-then-poll needs exact status matching, safer as plain Python.

Usage::

    omni-ingest ingestion/period_plan_chapter_grouping.yaml \\
...
async def run_payload(session: aiohttp.ClientSession, payload: dict[str, Any], cfg: ApiConfig) -> dict[str, Any]:
    ...
    for _ in range(cfg.max_poll_retries):
        ...
        except (aiohttp.ClientError, asyncio.TimeoutError) as exc:  # transient network error; keep polling
