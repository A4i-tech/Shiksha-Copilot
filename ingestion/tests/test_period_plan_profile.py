...
runs it through the omni-ingest pipeline using only the xlsx-read
...
PROFILE = Path(__file__).resolve().parents[1] / "period_plan_chapter_grouping.yaml"
...
def test_period_plan_chapter_grouping_drops_rows_before_first_chapter():
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Grade 6"
    ws.append(["Chapter Number", "Chapter title", "Period", "Topic", "Learning Outcomes"])
    ws.append([None, None, "P0", "Preface", "N/A"])  # front matter, no chapter number yet
    ws.append([1, "Knowing Our Numbers", "P1", "Large Numbers", "LO1"])
    out = io.BytesIO()
    wb.save(out)
    out.seek(0)

    chapters = _run_profile(out.read(), grade="Grade 6", subject="math")["chapters"]
    assert len(chapters) == 1, f"expected the null-chapter_number row to be dropped, got {chapters}"
